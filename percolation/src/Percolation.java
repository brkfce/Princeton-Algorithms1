...
public class Percolation {
    ...
    public Percolation(int n) {
        ...
        gridSize = 2 + (gridLength * gridLength); //The two virtual nodes, plus the number of squares in the grid
        gridConnect = new WeightedQuickUnionUF(gridSize);   //This datastructure will store the connections

        for (int i = 1; i <= gridLength; i = i++) {
            gridConnect.union(0, i);    //Unions the top row of the grid with the virtual top node
            gridConnect.union(gridSize, (gridSize - i));    //Likewise with the bottom row
        }
        noOfOpen = 0;
    }

    public void open(int row, int col) {    //Takes a square, and changes its value such that is open and connected
        validArgs(row, col);
        if (!isOpen(row, col)) {     //Does not need to open, if already open
            ...
            if (row != 1) {  //Connects the square to its neighbours, as long as it is not on an edge
    ...
    public boolean isFull(int row, int col) {   //Checks if the square is connected to the virtual top node
        validArgs(row, col);
        int gridPos = findGridPos(row, col);
    ...
    public boolean percolates() {   //Percolates if virtual top node connects to virtual bottom node
        return (gridConnect.find(gridSize) == gridConnect.find(0));
    }
    ...
    private int findGridPos(int row, int col) {
        return (1 + ((row - 1) * gridLength) + col);    //Finds the gridConnect index of the square;
    }
}
